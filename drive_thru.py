...
def main():
    ...
    arrival=list(filter(lambda x : x >= 0, np.random.normal(mean_selang_kedatangan, standard_deviation_selang_kedatangan, size))) 
    ...
    env.process(drive_thru(env, arrival, server_1, server_2, walking, data_record))
    env.run(until=90)
